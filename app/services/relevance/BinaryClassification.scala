...
object BinaryClassification {
  ...
  def predictRelevance (recs : List[Record], dataType : String, srch : Search) : Array[List[Record]] = {
    if (recs == null || recs.isEmpty) return Array.fill[List[Record]](2)(List[Record]())
    val dataSet =
      if (dataType == "Publication") publicationTrainingSet(publicationTrainingData(srch))
      else if (dataType == "Grant") grantTrainingSet(grantTrainingData(srch))
      else if (dataType == "Patent") patentTrainingSet(patentTrainingData(srch))
  ...
  def publicationTrainingData(search: Search) : List[(Publication, Boolean)] = {
  ...
  def grantTrainingData(search: Search) : List[(Grant, Boolean)] = {
    val driver = "org.postgresql.Driver"
    val url = "jdbc:postgresql://localhost/zweng4"
    val username = "zweng4"
    val password = ""
  ...
  def patentTrainingData( search: Search) : List[(Patent, Boolean)] = {
    val driver = "org.postgresql.Driver"
    val url = "jdbc:postgresql://localhost/zweng4"
    val username = "zweng4"
    val password = ""
