...
object LeanredPairwiseMergability {
  ...
  def link(recs : List[Record], dataType : String) : List[Record] = {
    if (recs == null || recs.isEmpty) return recs
    val dataSet =
      if (dataType == "Publication") publicationTrainingSet(publicationTrainingData())
      else if (dataType == "Grant") grantTrainingSet(grantTrainingData())
      else if (dataType == "Patent") patentTrainingSet(patentTrainingData())
  ...
  def publicationTrainingData() : List[(Publication, Publication, Boolean)] = {
  ...
  def grantTrainingData () : List[(Grant, Grant, Boolean)] = {
  ...
  def patentTrainingData () : List[(Patent, Patent, Boolean)] = {
    val driver = "org.postgresql.Driver"
    val url = "jdbc:postgresql://localhost/zweng4"
    val username = "zweng4"
    val password = ""
